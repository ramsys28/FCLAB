function [out]=showcs(data,locs,pars,hp);
...
if nargin>2
 ...
 if isfield(pars,'colormap')
        mycolormap=pars.colormap;
 end
...
for chan=1:n
  ...
  %subplot(nplot,50,chan);
  axes('Position',[faktor*locs(chan,4)*.85+.45 faktor*(locs(chan,5)*1.+.01)+.45 wfaktor wfaktor*.088/.07],'Parent',hp);
...
axes('Position',[0.12 0.01 .7/.88 .99],'Parent',hp);
...
Z = griddata(x,y,double(z),X,Y,'v4');
