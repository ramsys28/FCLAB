function pcorr=corrscan_fast(y1,y2,st,scanofinterest,k,norder) %l is the number of scans
...
for i=1:m
    ...
    y1new(i,1:scan(i))=y1(sum(scan(1:i-1,:))+1:sum(scan(1:i,:)));
    y2new(i,1:scan(i))=y2(sum(scan(1:i-1,:))+1:sum(scan(1:i,:)));
    %a=median(y1new(i,:));
    %b=median(y2new(i,:));
